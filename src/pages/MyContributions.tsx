...
const statusConfig: Record<PlantStatus, { icon: string; label: string; color: string }> = {
...
export default function MyContributions() {
  ...
  const verified = plants.filter((p) => p.status === "approved");
  const pending = plants.filter((p) => p.status === "pending");
  const rejected = plants.filter((p) => p.status === "rejected");
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#d8f3dc] p-4">
          <div className="grid grid-cols-4 divide-x divide-[#d8f3dc]">
            <div className="text-center px-2">
              <p className="font-display font-bold text-2xl text-[#2d6a4f]">{plants.length}</p>
              <p className="text-xs text-[#95d5b2]">Total</p>
            </div>
            <div className="text-center px-2">
              <p className="font-display font-bold text-2xl text-green-600">{verified.length}</p>
              <p className="text-xs text-[#95d5b2]">Verified</p>
            </div>
            <div className="text-center px-2">
          ...
          {verified.length > 0 && (
            <div className="mt-4 pt-4 border-t border-[#d8f3dc]">
              <BadgeList count={verified.length} />
            </div>
          )}
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {plants.map((p) => {
              ...
              return (
                <div key={p.id} className="bg-white rounded-2xl border border-[#d8f3dc] p-4 flex gap-3 shadow-sm">
                  {/* Image */}
                  <div className="w-16 h-16 rounded-xl overflow-hidden bg-[#d8f3dc] flex-shrink-0">
                    {p.photo_url ? (
                      <img src={p.photo_url} alt={p.plant_name} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-display font-semibold text-[#1a4731]">{p.plant_name}</h3>
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full border font-medium flex-shrink-0 ${cfg.color}`}
                      >
                        {cfg.icon} {cfg.label}
                      </span>
                    </div>
                    {p.landmark && (
                      <p className="text-xs text-[#95d5b2] mt-0.5">📌 Near {p.landmark}</p>
                    )}
                    {p.status === "approved" && (
                      <p className="text-xs text-green-600 font-semibold mt-1">
                        +{5 + (p.points_awarded ?? 10)} points earned
                      </p>
                    )}
                    {p.status === "pending" && (
